...
- **A1**: DIP를 적용하면 고수준 모듈이 저수준 모듈에 직접 의존하지 않고, 추상화된 인터페이스에 의존하게 된다. 이를 통해, 저수준 모듈이 변경되더라도 고수준 모듈에 영향을 주지 않으며, 시스템이 더 유연하고 확장 할 수 있다. 예를 들어, 새로운 종류의 로봇을 추가할 때 `RemoteControl` 클래스를 수정할 필요 없이, `새로운 로봇` 클래스만 인터페이스를 구현하면 됩니다
